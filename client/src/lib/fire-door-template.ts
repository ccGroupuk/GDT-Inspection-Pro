...
export const FIRE_DOOR_FIELDS: StepField[] = [
    ...
    { name: "certification_visible", label: "Certification Label Visible?", type: "select", options: ["Yes", "No", "Paint over", "Non readable"], required: true },
    { name: "resistance_rating", label: "Fire Resistance Rating", type: "select", options: ["FD30", "FD30s", "FD60", "FD60s", "Nominal 30", "Nominal 60", "N/A"], required: true },

    // Dimensions
    { name: "door_width", label: "Leaf Width (mm)", type: "number", required: true },
    { name: "door_height", label: "Leaf Height (mm)", type: "number", required: true },
    { name: "door_thickness", label: "Thickness (mm)", type: "number", placeholder: "e.g. 44 or 54", required: true },

    // Gaps - Hinge Side
    { name: "gap_hinge_top", label: "Hinge Gap Top (mm)", type: "number", required: true },
    { name: "gap_hinge_mid", label: "Hinge Gap Middle (mm)", type: "number", required: true },
    { name: "gap_hinge_bot", label: "Hinge Gap Bottom (mm)", type: "number", required: true },

    // Gaps - Leading Edge & Top
    { name: "gap_leading", label: "Leading Edge Gap (mm)", type: "number", required: true },
    { name: "gap_top", label: "Top Gap (mm)", type: "number", required: true },
    { name: "gap_bottom", label: "Bottom Gap / Threshold (mm)", type: "number", required: true },
    ...
    { name: "intumescent_seals", label: "Intumescent Seals", type: "select", options: ["Intact", "Paint Bound", "Missing/Damaged", "Wrong Size", "N/A"], required: true },
    { name: "smoke_seals", label: "Cold Smoke Seals", type: "select", options: ["Intact", "Damaged", "Missing", "N/A"], required: true },

    // E. Hardware
    { name: "hinges_grade", label: "Hinge Grade/Condition", type: "select", options: ["Grade 13 (Good)", "Grade 11 (Good)", "Worn/Leaking", "Missing Screws", "Non-compliant"], required: true },
    { name: "closer_op", label: "Door Closer Operational?", type: "select", options: ["Yes - Closes fully", "No - Fails to close", "No - slams", "N/A"], required: true },
    { name: "latch_op", label: "Latch/Lock Operational?", type: "select", options: ["Yes", "No", "N/A"], required: true },
